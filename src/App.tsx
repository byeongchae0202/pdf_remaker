...
const translations = {
  ...
  日本語: { upload: 'PDFをアップロード', select: 'ページを選択', order: '順番を整える', summary: 'AI要約', optional: '任意', create: 'PDFを作成', selected: '選択済み', emptyPages: 'アップロードしたPDFのページがここに表示されます。', emptySelection: '選択したページが順番に表示されます。', choose: '選択', summaryRequest: '要約を依頼 ↗', preview: '完成したPDFのプレビュー', pagePreview: 'ページプレビュー', pagePreviewButton: '表示', close: '閉じる', download: 'ダウンロード ↗', language: '表示言語', intro: 'PDFをまとめ、必要なページを選び、好きな順番で一つのファイルにします。', summaryModes: ['統合要約', 'PDF別要約', 'ページ別要約'], uploadHint: '複数のファイルを一度に選択できます', pagePlaceholder: '例: 1-3, 7', selectedPages: '選択したページ', pages: 'ページ', creating: '作成中...', privacy: 'AI要約とOCRでは文書がAzure AIに一時送信されます。データ処理に同意します。', encryptedTitle: 'ファイルのパスワード', encryptedHint: 'ページを表示するにはパスワードが必要です。', cancel: 'キャンセル', confirm: '確認', dragHint: 'PDFをドラッグまたはクリック' },
  简体中文: { upload: '上传 PDF', select: '选择页面', order: '调整顺序', summary: 'AI 摘要', optional: '可选', create: '创建 PDF', selected: '已选择', emptyPages: '上传的 PDF 页面会显示在这里。', emptySelection: '选中的页面会按顺序显示在这里。', choose: '选择', summaryRequest: '请求摘要 ↗', preview: '生成的 PDF 预览', pagePreview: '页面预览', pagePreviewButton: '预览', close: '关闭', download: '下载 ↗', language: '界面语言', intro: '上传 PDF，选择需要的页面，并按自己的顺序合并为一个文件。', summaryModes: ['综合摘要', '按 PDF 摘要', '按页面摘要'], uploadHint: '可以一次选择多个文件', pagePlaceholder: '例如：1-3, 7', selectedPages: '已选页面', pages: '页', creating: '创建中...', privacy: '使用 AI 摘要和 OCR 时，文档会临时发送到 Azure AI。我同意数据处理。', encryptedTitle: '文件密码', encryptedHint: '查看页面需要输入密码。', cancel: '取消', confirm: '确认', dragHint: '拖动或点击以添加 PDF' },
  繁體中文: { upload: '上傳 PDF', select: '選擇頁面', order: '調整順序', summary: 'AI 摘要', optional: '選用', create: '建立 PDF', selected: '已選擇', emptyPages: '上傳的 PDF 頁面會顯示在這裡。', emptySelection: '選取的頁面會依序顯示在這裡。', choose: '選擇', summaryRequest: '請求摘要 ↗', preview: '產生的 PDF 預覽', pagePreview: '頁面預覽', pagePreviewButton: '預覽', close: '關閉', download: '下載 ↗', language: '介面語言', intro: '上傳 PDF，選擇需要的頁面，並依照自己的順序合併成一個檔案。', summaryModes: ['整合摘要', '依 PDF 摘要', '依頁面摘要'], uploadHint: '可以一次選擇多個檔案', pagePlaceholder: '例如：1-3, 7', selectedPages: '已選頁面', pages: '頁', creating: '建立中...', privacy: '使用 AI 摘要和 OCR 時，文件會暫時傳送至 Azure AI。我同意資料處理。', encryptedTitle: '檔案密碼', encryptedHint: '檢視頁面需要輸入密碼。', cancel: '取消', confirm: '確認', dragHint: '拖曳或點擊以新增 PDF' },
} as const
